...
def MomentMatrix_construction(d):
    ...
    #    return id_probs[0] if c == w else id_probs[c]
...
def BD_MomMat(MomMat,id_els,d):
    ...
    return Trans, ListBlocks, P, id_els
...
Trans, list_blocks, P, id_els = BD_MomMat(MomMat,id_els,d)

# Write the SDP in the new block-diagonalized form
Nvars = 3
[dr, ds] = cp.Variable(2)
qcz = cp.Variable((nC,nZ),nonneg=True)

# List of values of the identifiable elements (id_els)
values = [ d, 1.0, 0.0, dr, d*ds, dr/d, 1.0/d]
values += [ qcz[c][z] for c in range(nC) for z in range(nZ) ]
...
# Moment matrix splitted into distinct blocks
G = {}
for l in range(len(list_blocks)):    
    G[l] = sum([ values[s] * Q[l][s] for s in range(len(id_els)) ])

# List the SDP constraints
ct = [ G[l] >> 0.0 for l in range(len(list_blocks)) if list_blocks[l] > 1.0 ]
ct += [ sum([ qcz[c][z] for c in range(nC) ]) == 1.0 for z in range(nZ) ] 

# Build and solve the SDP using CVXPY and MOSEK
#goal = sum([ qcz[c][c]/(d) for c in range(nC)])
goal = sum([ qcz[0][z]/(d+1) for z in range(nZ)])
...
# Present the results 
print('RESULT FOR DIMENSION',d,'IS:',goal.value)
